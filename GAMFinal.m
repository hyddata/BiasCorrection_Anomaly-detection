...
abs_errors_cal=abs(errors_cal);
abs_errors_val=abs(errors_val);
RMSE_cal=round(sqrt(mean(errors_cal(:).^2)),2);
RMSE_val=round(sqrt(mean(errors_val(:).^2)),2);
MAE_cal=(round(mean(abs_errors_cal(:)),2));
MAE_val=round(mean(abs_errors_val(:)),2);
...
bar(time_indices, bias_corrected_data, 'k'); hold on;
...
subplot(2, 1, 2); 
bar(unique_years, annual_means, 'k'); hold on;
